...
public class CoreServerModule
{
            ...
            LocalDatabase localDatabase, MessageLogger<MemberId> messageLogger, Supplier<DatabaseHealth> dbHealthSupplier, SslPolicy sslPolicy )
    {
        ...
        CatchUpClient catchUpClient = life.add( new CatchUpClient( logProvider, Clocks.systemClock(), inactivityTimeoutMillis, monitors, sslPolicy ) );

        RemoteStore remoteStore = new RemoteStore( logProvider, fileSystem, platformModule.pageCache,
                new StoreCopyClient( catchUpClient, logProvider, clusteringModule.topologyService() ),
                new TxPullClient( catchUpClient, platformModule.monitors, clusteringModule.topologyService() ), new TransactionLogCatchUpFactory(),
        ...
        CoreStateDownloader downloader =
                new CoreStateDownloader( localDatabase, servicesToStopOnStoreCopy, remoteStore, catchUpClient, logProvider, storeCopyProcess,
                        coreStateMachinesModule.coreStateMachines, snapshotService, commandApplicationProcess, clusteringModule.topologyService() );
